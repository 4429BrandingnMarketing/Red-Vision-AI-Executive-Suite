...
function pcmToWavBuffer(pcmBase64: string, sampleRate = 24000, numChannels = 1, bitsPerSample = 16): Buffer {
...
Output ONLY the style brief text — no labels, no quotes, no preamble.`;
  ...
  // Endpoints for Voiceover Engine (gemini-3.6-flash & gemini-3.1-flash-tts-preview)
  app.post("/api/generate-voiceover-script", async (req, res) => {
    try {
      const { topic, format, voiceTone, emotion } = req.body;
      const ai = getAiClient();
      const systemInstruction = `You are a Hollywood entertainment director and master social-media copywriter.
Create an engaging, viral voiceover narration script tailored for ${format || 'Instagram Reel (15s)'}.
Include explicit speech delivery cues (such as "Say enthusiastically:", "Say with deep gravitas:") at the start of sentences.
Return JSON with two keys:
1. "script": The exact text script ready for TTS synthesis.
2. "pacingNotes": Short timing and emotion guidelines (1-2 sentences).`;

      const response = await ai.models.generateContent({
        model: "gemini-3.6-flash",
        contents: `Topic/Concept: ${topic}\nFormat: ${format}\nVoice Tone: ${voiceTone}\nEmotion: ${emotion}`,
        config: {
          systemInstruction,
          responseMimeType: "application/json",
          responseSchema: {
      ...
      res.status(500).json({ error: error.message || "Failed to generate voiceover script" });
    }
  });

  app.post("/api/generate-voiceover-audio", async (req, res) => {
    try {
      const { scriptText, voiceName = 'Puck', isMultiSpeaker = false } = req.body;
      ...
      // Convert PCM base64 to WAV buffer with 24kHz 16-bit header
  ...
  // Complete One-Prompt AI Agency Campaign Generator Endpoint
  app.post("/api/generate-agency-campaign", async (req, res) => {
    try {
      const { prompt, niche, targetClient } = req.body;
      if (!prompt || !prompt.trim()) {
        return res.status(400).json({ error: "Campaign prompt is required" });
      }
      const ai = getAiClient();

      const systemInstruction = `You are a world-class AI Ad Agency Creative Director and Copywriter (Claude Opus & Gemini 3.5 class intelligence).
Given a single campaign prompt or client business idea, generate a complete end-to-end ad agency campaign bundle in strict JSON format.
The bundle must contain:
1. High-converting ad copy (hooks, headlines, primary body copy, CTAs, hashtags).
2. Ad image concepts with specific visual prompts, overlay headlines, and badge callouts.
3. High-converting video ad concepts with storyboard scenes (hook, problem, solution, CTA) and a full voiceover script.
4. Client outreach scripts (cold email, Instagram DM, LinkedIn pitch, cold call script) to sell this exact campaign to paying local or e-commerce clients for $500 - $5,000/mo.
5. Agency proposal package details and estimated ROI.`;

      const response = await ai.models.generateContent({
        model: "gemini-3.6-flash",
        contents: `Campaign Prompt: ${prompt}\nNiche Focus: ${niche || 'General Business/E-Commerce/Local'}\nTarget Client Type: ${targetClient || 'Local Business or E-Commerce Store'}`,
        config: {
          systemInstruction,
          responseMimeType: "application/json",
          responseSchema: {
            ...
            properties: {
              ...
              suggestedRetainerPrice: { type: Type.STRING },
              adCopy: {
                type: Type.OBJECT,
                properties: {
                  hooks: {
                    ...
                    items: {
                      ...
                      properties: {
                        ...
                        text: { type: Type.STRING },
                        conversionScore: { type: Type.STRING }
                      },
                      required: ["type", "text", "conversionScore"]
                    }
                  },
                  headlines: {
                    type: Type.ARRAY,
                  ...
                  primaryBodyCopy: { type: Type.STRING },
                  ctaText: { type: Type.STRING },
                  hashtags: { type: Type.STRING }
                },
                required: ["hooks", "headlines", "primaryBodyCopy", "ctaText", "hashtags"]
              },
              imageCreatives: {
                type: Type.ARRAY,
                items: {
                  ...
                  required: ["title", "aspectRatio", "visualConcept", "overlayHeadline", "badgeText", "imagePrompt"]
                }
              },
              videoCreatives: {
                type: Type.ARRAY,
                  ...
                  properties: {
                    ...
                    storyboard: {
                      ...
                      items: {
                        ...
                        required: ["time", "scene", "voiceover"]
                      }
                    }
                  },
                  required: ["title", "duration", "aspectRatio", "hookVisual", "voiceoverScript", "storyboard"]
                }
              },
              clientOutreach: {
                type: Type.OBJECT,
                properties: {
                  ...
                  instagramDM: { type: Type.STRING },
                  linkedInPitch: { type: Type.STRING },
                  coldCallScript: { type: Type.STRING }
                },
                required: ["coldEmailSubject", "coldEmailBody", "instagramDM", "linkedInPitch", "coldCallScript"]
              },
              proposalSummary: {
                type: Type.OBJECT,
                properties: {
                  agencyPackageName: { type: Type.STRING },
                  monthlyRetainer: { type: Type.STRING },
                ...
                required: ["agencyPackageName", "monthlyRetainer", "includedDeliverables", "expectedROI"]
              }
            },
            required: [
              "campaignTitle", "niche", "targetAudience", "uniqueValueProp", 
              "suggestedRetainerPrice", "adCopy", "imageCreatives", 
              "videoCreatives", "clientOutreach", "proposalSummary"
            ]
          }
        }
      });
      ...
      let raw = response.text || "{}";
      raw = raw.replace(/```(json)?/gi, '').trim();
      const campaignData = JSON.parse(raw);
      res.json(campaignData);
    } catch (error: any) {
      console.error("Error generating agency campaign:", error);
